class WorkingTimesController < ApplicationController
  ...
  def index
    ...
    @working_times = current_user.working_times.where(start_time: @start_date.beginning_of_month..@start_date.end_of_month)
  end
  ...
  def weekly_index
    ...
    @working_times = current_user.working_times.where(start_time: @day.beginning_of_week..@day.end_of_week).order(start_time: :asc)
  end
